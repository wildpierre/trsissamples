...
app.controller("SchoolsController", function ($scope, $http) {
    ...
    $scope.successGetSchoolHistoryCallback = function (response) {
        $scope.schoolHistoryList = response.data;
                
        for (var i = 0; i < $scope.schoolHistoryList.length; i++) {            
            $scope.schoolHistoryList[i].userFriendlyDate=new Date($scope.schoolHistoryList[i].date).toLocaleString();
        }       
        
        $scope.errormessage = "";
    };

    $scope.errorGetSchoolHistoryCallback = function (error) {
        console.log(error);
        $scope.errormessage = "Unable to get changes log";
    };

    $scope.getSchoolHistory = function () {
        $http.get('/public/rest/history/schools/').then($scope.successGetSchoolHistoryCallback, $scope.errorGetSchoolHistoryCallback);
    };    
    ...
    $scope.deleteSchool = function (id) {
        ...
        $http.delete('/rest/schools/' + id).then($scope.successDeleteSchoolCallback, $scope.errorDeleteSchoolCallback);
    };
    ...
    $scope.addSchool = function () {
        $http.post('/rest/schools/' + $scope.inputnumber + "/" + $scope.inputname).then($scope.successAddSchoolCallback, $scope.errorAddSchoolCallback);
    };
